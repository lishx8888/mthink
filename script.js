...
class MindMap {
    constructor(canvasId) {
        ...
        // 连接线和箭头的统一颜色
        this.connectionColor = '#b3b875ff'; // 默认颜色 R:183 G:106 B:85
                        ...
                        // 确保节点路径应用了正确的样式
                            ...
                            if (nodePath) {
                                ...
                                if (node.style.borderStyle === 'none') {
                                    ...
                                    nodePath.setAttribute('stroke-dasharray', node.style.borderStyle === 'none' ? 'none' : (node.style.borderStyle === 'solid' ? 'none' : (node.style.borderStyle === 'dashed' ? '5,5' : '2,2')));
                                }
    ...
    render() {
        ...
        const renderNodeHelper = (node) => {
            ...
            if (node.style.borderStyle === 'none') {
                ...
                rectPath.setAttribute('stroke-dasharray', node.style.borderStyle === 'none' ? 'none' : (node.style.borderStyle === 'solid' ? 'none' : (node.style.borderStyle === 'dashed' ? '5,5' : '2,2')));
            }
        ...
        if (rightClickArea) {
            ...
            textDiv.style.textAlign = node.style.textAlign || 'left';
            textDiv.style.display = 'block'; // 使用block布局，确保自动换行正常工作
            textDiv.style.wordWrap = 'break-word';
            textDiv.style.whiteSpace = 'pre-wrap'; // 使用pre-wrap，保留换行符并让浏览器自动换行
            ...
            // 使用原始文本，让浏览器根据当前的字体和容器宽度自动计算换行位置
    ...
    calculateNodeSizes() {
        ...
        const padding = 10; // 节点内边距，与编辑模式保持一致
    ...
    editNodeText(node) {
        ...
        editableDiv.className = 'edit-textarea';
        editableDiv.textContent = node.text;
        ...
        editableDiv.style.fontSize = `${node.style.fontSize}px`;
        editableDiv.style.fontFamily = node.style.fontFamily;
        ...
        // 创建div容器，使用与显示模式完全一致的布局
        const textDiv = document.createElement('div');
        textDiv.style.width = '100%';
        textDiv.style.textAlign = node.style.textAlign || 'left';
        textDiv.style.display = 'block'; // 使用block布局，与显示模式一致
        textDiv.style.wordWrap = 'break-word';
        textDiv.style.whiteSpace = 'pre-wrap'; // 使用pre-wrap，与显示模式一致
        textDiv.style.overflowWrap = 'break-word'; // 更现代的属性，确保长单词自动换行
        textDiv.style.hyphens = 'auto'; // 启用自动连字符
        textDiv.style.padding = '0'; // 与显示模式一致，无内边距
        textDiv.style.margin = '0';
        
        // 添加可编辑div到textDiv
        textDiv.appendChild(editableDiv);
        
        // 添加到foreignObject
        textInput.appendChild(textDiv);
        ...
        // 自动调整文本区域和节点大小
        const autoResize = () => {
            // 不调整editableDiv高度，保持为100%，确保文字与上下边框的距离一致
            
            // 使用Canvas测量文本宽度
            const ctx = document.createElement('canvas').getContext('2d');
            ctx.font = `${node.style.fontSize}px ${node.style.fontFamily}`;
            
            const originalLines = editableDiv.textContent.split('\n');
            const wrappedLines = [];
            const lineHeight = parseInt(node.style.fontSize) * 1.4;
            const padding = 10; // 与calculateNodeSizes函数保持一致
            const fixedNodeWidth = 400; // 固定节点宽度，与calculateNodeSizes函数保持一致
            const contentWidth = fixedNodeWidth - padding * 2; // 节点内容的宽度
            ...
            // 不调整editableDiv大小，保持为100%，确保文字与上下边框的距离一致
        ...
        const handleGlobalMouseDown = (e) => {
            ...
            if (nodeGroup.contains(e.target)) {
                // 如果点击的是节点组内部（但不是editableDiv），不做任何操作
                return;
            }
        ...
        editableDiv.addEventListener('input', autoResize);
    ...
    updateStylePanel() {
        ...
        if (this.selectedNodes.length === 1) {
            ...
            // 更新文本对齐按钮的选中状态
            const textAlignOptions = document.querySelectorAll('.text-align-option');
            textAlignOptions.forEach(function(option) {
                option.classList.remove('selected');
                if (option.getAttribute('data-align') === (node.style.textAlign || 'left')) {
            ...
            shapeOptions.forEach(function(option) {
                option.classList.remove('selected');
                if (option.getAttribute('data-shape') === node.style.shape) {
                    option.classList.add('selected');
                }
            });
        } else {
    ...
    updateNodeStyle(updatedProperties, shape) {
        ...
        this.selectedNodes.forEach(node => {
            ...
            if (updatedProperties.includes('textAlign') && stylePanelValues.textAlign !== node.style.textAlign) {
                updatedStyle.textAlign = stylePanelValues.textAlign;
            }
    ...
    initEventListeners() {
        ...
        textAlignOptions.forEach(function(option) {
            option.addEventListener('click', function() {
                // 移除所有选项的选中状态
                textAlignOptions.forEach(opt => opt.classList.remove('selected'));
                // 添加当前选项的选中状态
                this.classList.add('selected');
                // 获取选中的对齐方式
                ...
                if (textAlignInput) {
                    ...
                    // 更新选中节点的文本对齐方式
                    self.updateNodeStyle(['textAlign']);
                }
            });
        });
