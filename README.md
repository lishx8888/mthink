...
- **触屏编辑优化**：修复了触屏设备上的编辑问题，包括长按节点进入编辑模式后，再长按其它节点时没有进入当前节点的编辑模式的问题，以及编辑内容后点击其他节点或空白位置内容消失的问题
- **节点删除功能**：在右侧节点栏中添加了节点删除按钮，位于手形工具的右侧，使用自定义的删除图标
- **撤销重做功能**：在右侧文件栏中添加了撤销和重做功能按钮，位于导出按钮的右侧，使用自定义的撤销和重做图标
...
11. 使用**撤销**按钮撤销上一步操作
12. 使用**重做**按钮重做被撤销的操作
13. 选择节点后，使用**删除**按钮删除选中的节点
14. **右侧栏显隐控制**：将鼠标移动到页面右下角的90x90px方形区域，会显示一个带有"右"字的圆形按钮，点击该按钮可以隐藏或显示右侧栏
